...
- 修复启用或停用 Profile 触发 SIM Refresh 后，界面立即刷新并报 `EuiccChannel not found` 的问题。Lumina 现在会等待 OMAPI 通道重新连接后再读取配置。
- 通道仍在恢复时会自动进行一次有限重试；超过恢复时间后改为显示本地化的重连说明和“重试”按钮，不再向用户暴露原生异常堆栈。
- 新增经确认的密集 Profile Dashboard 视觉基线：每张配置仅保留一个国旗，不再重复显示运营商头像和标题国旗，为后续真实数据界面迭代提供回归基准。本项是设计预览，不改变当前生产 Profile 卡片的数据来源。
