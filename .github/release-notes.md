...
- 将首页仪表盘、通道选择、eUICC 身份摘要、配置卡片和软件更新页面拆为职责明确的 Flutter 组件；既有交互、公开 API 与测试 key 保持兼容。
- 统一通道、Profile 和 eUICC 信息的刷新入口，减少页面间重复编排 provider 刷新的维护风险。
- 补充仪表盘组件测试，并修复 USB 通道标签与类型详情同名时的测试定位歧义。
- 本版本聚焦可维护性和回归稳定性，不扩大任何白卡、Profile 写操作或 USB CCID 的真实硬件验证范围。

## 现有能力与边界

- Profile 列表接口不提供单个配置的精确占用大小；仅在公开观测样本与当前 EID 芯片族可靠匹配时显示“约 xx KiB”，无法可靠匹配时隐藏该字段，不会把估算值冒充卡片测量值。
- 保号提醒使用 Android AlarmClock 调度和高优先级“eSIM 保号闹钟”通道；未获得精确闹钟权限时仍保留可恢复的非精确调度并提示用户开启权限。首页顶部铃铛只展示本机保号提醒，卡片侧待上报技术事件在设置页单独展示。
- 保留密集配置仪表盘、单国旗、ICCID 脱敏、启停互斥、微流量检查、重命名和删除等既有能力。
- **设置 → 软件更新**仅检查本项目官方不可变 GitHub Release，自动选择与当前安装一致的 APK 架构，下载后校验 GitHub SHA-256、包名、新版本和完整四签名集合，再交给 Android 系统安装器确认。
