...
- 新增 **设置 → 软件更新**：仅检查本项目官方不可变 GitHub Release，自动选择与当前安装一致的 APK 架构，下载后校验 GitHub SHA-256、包名、新版本和完整四签名集合，再交给 Android 系统安装器确认。
- 更新不是静默安装；Android 仍会要求用户确认，并可能先要求为 Lumina 开启“安装未知应用”。`0.1.2` 本身没有更新器，因此升级到本版本仍需手动下载安装一次。
