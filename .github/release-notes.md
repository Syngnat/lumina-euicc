...
- 将已确认的密集配置仪表盘接入生产首页：集中展示当前通道、脱敏 EID、真实可用 NVRAM、通知入口和新增 eSIM 入口。
- Profile 卡片进一步压缩，一屏可查看更多配置；每张卡片只显示一个国家或地区旗帜，ICCID 默认脱敏并支持长按复制。
- 启用与停用改为卡片内直接开关，增加单任务互斥、等待状态、失败提示和操作后的真实数据刷新，避免重复点击触发并发切换。
- 保号提醒、微流量保号检查、重命名、删除等既有操作继续保留在紧凑卡片中。当前 LPA 数据模型不提供单个 Profile 的可靠占用大小，因此界面明确显示“大小未知”，不会伪造容量数据。
- 保留 **设置 → 软件更新**：仅检查本项目官方不可变 GitHub Release，自动选择与当前安装一致的 APK 架构，下载后校验 GitHub SHA-256、包名、新版本和完整四签名集合，再交给 Android 系统安装器确认。
- 本版本的 Profile 启停仍需更多真实白卡验证；界面与桥接路径完成不等于所有卡片、固件和手机组合均已通过写操作验证。
