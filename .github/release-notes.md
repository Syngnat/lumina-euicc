...
- Profile 菜单新增 **保号提醒**：可设置、修改或取消日期与时间，并在卡片底部显示下一次提醒时间。
- 到时由 Android 系统发送本地通知；手机重启、应用升级或进程重启后会自动恢复调度。删除 Profile 会一并取消提醒，重命名后通知名称会同步更新。
- Android 13+ 会按需请求通知权限；Android 12+ 未授予“闹钟和提醒”特殊权限时仍会保存日期，并自动降级为可能稍有延迟的非精确提醒。
- 提醒数据仅保存在本机应用私有目录；Profile ICCID 只用于生成不可逆的 SHA-256 本地键，不保存 ICCID 明文，也不会上传提醒数据。
- 保留 **设置 → 软件更新**：仅检查本项目官方不可变 GitHub Release，自动选择与当前安装一致的 APK 架构，下载后校验 GitHub SHA-256、包名、新版本和完整四签名集合，再交给 Android 系统安装器确认。
- 更新不是静默安装；Android 仍会要求用户确认，并可能先要求为 Lumina 开启“安装未知应用”。已安装 `0.1.3` 或后续版本的用户可以直接通过应用内更新升级到本版本。
