...
| Keep-alive reminders | Per-profile local date/time alarm implemented with Android AlarmClock scheduling, alarm-sound/vibration notification channel, reboot/app-update recovery, and inexact fallback; it stores no plaintext ICCID and never writes reminder data to the eUICC |
...
| Reminder center / eUICC reports | The home bell lists local keep-alive reminders; card-side pending report events are separately labelled in Settings, while native process/delete handlers are not exposed through the Dart API or UI |
...
The profile-list protocol does not expose exact per-profile storage usage.
Lumina shows an explicitly labelled estimate only when a pinned public sample
matches the provider and current EID family; otherwise the size field is hidden
instead of displaying or inventing a value.

The profile menu can set, edit, or cancel a local keep-alive alarm. Android 13+
asks for notification permission. Android 12+ uses AlarmClock scheduling when
the user allows “Alarms & reminders”; otherwise the alert remains scheduled as
an inexact alarm and may be delayed by the system. A dedicated alarm channel
uses the system alarm sound and vibration by default, subject to the user's
Android/ColorOS channel settings. Reminder metadata stays in the app's private
storage and is removed automatically when its profile is deleted.
