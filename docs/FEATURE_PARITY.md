...
| API | 说明 |
|---|---|
| `requestPhoneStatePermission` | 仅在用户明确启动操作后请求电话权限，用于把物理卡槽/端口映射到 Android 活动订阅 |
| `runMicroDataKeepAlive` | 仅支持手机 OMAPI 通道；必要时临时启用目标 Profile，为对应订阅申请专用蜂窝 `Network`，发送一次不跟随跳转的 HTTPS `HEAD` 请求，并在 `finally` 中恢复先前 Profile |

响应正文上限为 1 KiB（`HEAD` 正常为 0），完成后立即释放 Lumina 的
`NetworkCallback`。这不是全局数据开关，不能约束 DNS/TCP/TLS、系统后台
流量或运营商计费，也不能证明运营商已延长号码有效期。当前只有代码与
